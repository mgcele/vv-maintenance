...
Layer.fn = function() {
    ...
    this.warn = function(msg, callback) {
        this.hidePrevious();
        //
        layerIndex = layer.alert(msg, "warn", callback);
        //
        adjustMsgText();
        //
        return this;
    ...
    this.load = function(str) {
        var html = '<div align="center" style="width:120px;"><img src="resources/layer/skin/default/loading2-2.gif" /><br/><br/><p style="font-size:16px;letter-spacing:2px;color:#333300">'+str+'</p></div>';
        ...
        //
        var theLayer = $id("xubox_layer" + layerIndex);
        // var theText = theLayer.find("> div.xubox_main");
        theLayer.css("left", "33%");
        theLayer.css("top", "35%");
        delBtnsBar(true);
        //
        return this;
...
Layer.info = function(msg, callback) {
    return new Layer.fn().info(msg, callback);
};
Layer.warn = function(msg, callback) {
    return new Layer.fn().warn(msg, callback);
};
Layer.error = function(msg, callback) {
    return new Layer.fn().error(msg, callback);
};

Layer.progress = function(msg) {
    return new Layer.fn().progress(msg);
};
Layer.confirm = function(msg, yesCallback, noCallback) {
    return new Layer.fn().confirm(msg, yesCallback, noCallback);
};
Layer.prompt = function(config, yesCallback, noCallback) {
    return new Layer.fn().prompt(config, yesCallback, noCallback);
};
Layer.msg = function(msg, type, callback, closeDelay) {
    return new Layer.fn().msg(msg, type, callback, closeDelay);
};
Layer.msgInfo = function(msg, callback, closeDelay) {
    return new Layer.fn().msg(msg, "info", callback, closeDelay);
};
Layer.msgWarn = function(msg, callback) {
    return new Layer.fn().msg(msg, "warn", callback, false);
};
Layer.msgSuccess = function(msg, callback, closeDelay) {
    return new Layer.fn().msg(msg, "success", callback, closeDelay);
};
Layer.html = function(html) {
    return new Layer.fn().html(html);
};
Layer.tips = function(html, follow, config) {
    return new Layer.fn().tips(html, follow, config);
};
Layer.dialog = function(config) {
    return new Layer.fn().dialog(config);
};
Layer.load = function(str){
    return new Layer.fn().load(str);
};
